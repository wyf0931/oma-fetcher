function app() {
  return {
    ...
    detail: null, detailDialog: false, showAuthDialog: false, authInput: '', showKeyDialog: false, newKeyName: '', createdToken: '', tokenCache: {}, toast: '', toastType: 'success',
    ...
    async createKey() { try { const result = await this.apiClient.request('/api/keys', {method: 'POST', body: JSON.stringify({name: this.newKeyName.trim(), scopes: ['fetch', 'search']})}); this.createdToken = result.data.token; this.apiClient.saveToken(result.data.id, result.data.token); this.newKeyName = ''; await this.loadKeys(); this.notify('API key created; copy it now'); } catch (error) { this.handleError(error); } },
